...
function CodeBlock({ codeSamples, compact = false }: { codeSamples: Record<string, string>, compact?: boolean }) {
  ...
  return (
    <div className={`w-full rounded-lg border border-gray-200 dark:border-gray-800 bg-base-700 shadow-sm overflow-hidden`}>
      <div className={`flex items-center space-x-2 border-b border-base-600 dark:border-gray-800  dark:bg-[#161619]/50 px-3 ${compact ? 'py-1' : 'py-2'} text-xs`}>
        {languages.length > 1 ? (
          languages.map((lang) => (
            <button
              key={lang}
